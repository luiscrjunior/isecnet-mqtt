...
def mqtt_send_alarm_status(client: mqtt.Client):
    ...
    try:
        ...
        if response.is_published():
            logger.info("Status enviado para o tópico '%s': %s"%(config.mqtt_topic_alarm_status,payload))
    except Exception as err:
...
try:
    ...
    connection_status = client.connect(config.mqtt_addr, port=config.mqtt_port)
